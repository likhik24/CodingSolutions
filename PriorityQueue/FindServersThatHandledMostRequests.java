...
public class FindServersThatHandledMostRequests {
    public List<Integer> busiestServers(int k, int[] arrival, int[] load) {

         // use a tree to track available servers
         TreeSet<Integer> availableServerIds = new TreeSet<Integer>();
         for (int num = 0; num < k; num++) {
             availableServerIds.add(num);
         }
         // use a PQ to maintain the availability based on curTime + loadTime and the server index = idx%k
         Queue<int[]> runningServers = new PriorityQueue<>(Comparator.comparingInt(a -> a[0]));
         ...
         for (int idx = 0; idx < arrival.length; idx++) {
           ...
           //  peek if the server's work time is less than or equal to the next task completion time, if it is poll those servers from the running servers queue and add the index of that server to the availableServerIds treeSet
             while (!runningServers.isEmpty() && runningServers.peek()[0] <= newTaskCompletionTime) {
                 int freedServer = runningServers.poll()[1];
                 availableServerIds.add(freedServer);
             }

             if (availableServerIds.size() == 0) continue; // all busy

             // to always get the last freed server
             Integer serverIdx = availableServerIds.ceiling(idx % k);

             if (serverIdx == null) {
                 serverIdx = availableServerIds.first();
             }

             serverHandledReqCount[serverIdx]++;
             availableServerIds.remove(serverIdx);
