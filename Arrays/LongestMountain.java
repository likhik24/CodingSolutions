package Arrays;

// Longest Mountain in Array
//You may recall that an array arr is a mountain array if and only if:
//
//        arr.length >= 3
//        There exists some index i (0-indexed) with 0 < i < arr.length - 1 such that:
//        arr[0] < arr[1] < ... < arr[i - 1] < arr[i]
//        arr[i] > arr[i + 1] > ... > arr[arr.length - 1]
//        Given an integer array arr, return the length of the longest subarray, which is a mountain. Return 0 if there is no mountain subarray.
//
//        Example 1:
//
//        Input: arr = [2,1,4,7,3,2,5]
//        Output: 5
//        Explanation: The largest mountain is [1,4,7,3,2] which has length 5.
//        Example 2:
//
//        Input: arr = [2,2,2]
//        Output: 0
//mountain is formed by finding the peak such that left lowest of peak is increasing and right side of peak element is decreasing, following principle of loss of generality ,
// a peak cannot start until current mountain ends as it will be lowest that current mountain height
//Intuition
//
//        Without loss of generality, a mountain can only start after the previous one ends.
//
//        This is because if it starts before the peak, it will be smaller than a mountain starting previous; and it is impossible to start after the peak.

public class LongestMountain {
    public int findLongestMountain(int[] arr) {
        //follow two pointer technique to find left and right of the mountain
        int left=0;
        int right=0;
        int maxLen = 0;
        while(left<arr.length && right < arr.length) {
            right = left+1;
            if(right > arr.length-1 && arr[right+1] > arr[right] ) {
                while(right > arr.length-1 && arr[right+1] > arr[right] )
                    right++;
                while(right > arr.length-1 && arr[right] > arr[right+1] )
                    right++;
                maxLen = Math.max(maxLen, right-left+1);
            }
            left = Math.max(right, left+1);
        }
        return maxLen;
    }
}
